...
// Apply saved channel states to hardware
void applyChannelStates() {
    // Set pins according to saved states
    // Remember LOW is ON, HIGH is OFF for channels
    digitalWrite(CH1_PIN, channelStates[0] ? LOW : HIGH);
    digitalWrite(CH2_PIN, channelStates[1] ? LOW : HIGH);
    digitalWrite(CH3_PIN, channelStates[2] ? LOW : HIGH);
    digitalWrite(CH4_PIN, channelStates[3] ? LOW : HIGH);
}
        ...
        if (state == 0 || state == 1) {
            if (device == "CH1") {
                ...
                if (state == 0) {  // If turning ON
                    ch1ResetCount = 0;  // Reset the counter
                    lastResetTime = 0;   // Reset the timer
                }
                success = true;
...
void lastStatus() {
    ...
    Serial.print("F1:"); Serial.println(digitalRead(F1_PIN));
    Serial.print("F2:"); Serial.println(digitalRead(F2_PIN));
    Serial.print("F3:"); Serial.println(digitalRead(F3_PIN));
    Serial.print("F4:"); Serial.println(digitalRead(F4_PIN));
    Serial.print("IGN:"); Serial.println(digitalRead(IGN_PIN));
    Serial.print("TEMP:"); Serial.println(currentTemp, 2);
    Serial.print("BAT:"); Serial.println(batteryVoltage, 2);
    Serial.print("CH1:"); Serial.println(channelStates[0] ? "1" : "0");
    Serial.print("CH2:"); Serial.println(channelStates[1] ? "1" : "0");
    Serial.print("CH3:"); Serial.println(channelStates[2] ? "1" : "0");
    Serial.print("CH4:"); Serial.println(channelStates[3] ? "1" : "0");
    Serial.print("RELAY:"); Serial.println(RelayFlag ? "1" : "0");
    Serial.print("TEMP_THRESH:"); Serial.println(tempThreshold);
    Serial.print("TIME_THRESH_MIN:"); Serial.println(timeThreshold / 60000.0);
}